...
def get_color(orig_label=None,
  ...
  if ((gt_label is None or orig_label == gt_label) and
    ...
    color = '#023603'  # 'drak-green': correct prediction
...
def plot_images(images,
  ...
  for i in range(h):
    for j in range(w):
      ...
      if annotate_only_first_image and j == 0:
        annot_this = True 
      elif annotate_only_first_image is False:
        annot_this = True
      else:
        annot_this = False
      annot_this &= annots is not None
      ...
      if has_labels:
        color = get_color(orig_label, gt_label,
                          None, None, False)
        ax.annotate(
            str(orig_label),
            ...
            alpha=text_alpha,
            fontsize=(0.30 * 72 * figure_height / h)
        )
  ...
  if filename:
    with open('{filename}.{filetype}'.format(**locals()), 'w') as f:
...
def canvas_to_np(figure, rescale=False):
  ...
  buff, (width, height) = figure.canvas.print_to_buffer()
  img = np.frombuffer(buff, np.uint8).reshape(height, width, -1)
  if img.shape[-1] > 3:
    img = img[:, :, 0:3]
